...
ggplot(data = most_recent, mapping = aes(x = days_to_approval, y = (days_to_approval - all_linear_reg$fitted.values))) +
...
```{r}
most_recent_county <- counties_all_1028 %>%
   mutate(median_income = median_income / 1000,
         bank_ppp_issued = bank_ppp_issued / 1000,
         # This is b/c we didn't do per 10,000 people before, but we want hundreds of businesses per 10,000 people, or 100
         businesses_per_pop = businesses_per_pop * 100,
         violent_crime_rate = violent_crime_rate / 10,
         pop_den = pop_den / 100,
