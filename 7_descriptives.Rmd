...
sapply(most_recent, mean) %>%
  bind_rows() %>%
  pivot_longer(cols = loan_range:fips, names_to = "variable", values_to = "mean")
```
...
most_recent <- most_recent %>%
  select(business_days, median_family_income, minority_percent,
         ...
         unemployment_rate, mean_start_cases_weighted, mean_start_deaths_weighted,
         mean_end_cases_weighted, mean_end_deaths_weighted, stay_at_home_apr,
         stay_at_home_current, non_essential_closure_apr, non_essential_closure_current)


sapply(most_recent, sd) %>%
    bind_rows() %>%
  pivot_longer(cols = business_days:non_essential_closure_current, 
...
```{r}
# Calculating for business patterns
payroll <- 
