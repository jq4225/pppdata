...
most_recent_county <-
  readRDS('most_recent_county_1130.rds') %>%
  mutate(baml = ifelse(lender == "bank of america, national association",
                       1, 0)) %>%
  mutate(jpm = ifelse(lender == "jpmorgan chase bank, national association",
                       1, 0)) %>%
...
most_recent_both <- readRDS('most_recent_zip_withcounties_1130.rds') %>%
    mutate(baml = ifelse(lender == "bank of america, national association",
                       1, 0)) %>%
  mutate(jpm = ifelse(lender == "jpmorgan chase bank, national association",
                       1, 0)) %>%
...
# And this is for ownership share county regression -- at the bottom!

most_recent_ownership <- readRDS('most_recent_county_ownership_1203.rds') %>%
    mutate(baml = ifelse(lender == "bank of america, national association",
                       1, 0)) %>%
  mutate(jpm = ifelse(lender == "jpmorgan chase bank, national association",
                       1, 0)) %>%
...
# between the interaction terms. Ignore them!
...
```{r}
# Redoing county level with felm just to make sure everything's correct. This
# doesn't include state FEs but I'm using the same function for consistency
# there are slight diffs in calculated coefs with lm_robust vs. felm
...
```{r}
# banks county -- using only county data, again without FEs
...
# Robustness check w/ zip and county -- replacing ZIP estimates with the 
# county data, but keeping other data at ZIP level
...
baml_check <- felm(
  business_days ~ 1 + 
    ...
    non_essential_closure_apr +
    non_essential_closure_current + 
    baml + I(baml*minority_percent)|0|0|zip, 
    data = most_recent_both)
...
citi_check <- felm(
  business_days ~ 1 + 
    ...
    non_essential_closure_apr +
    non_essential_closure_current + 
       citi + I(citi * minority_percent)|0|0|zip, 
    data = most_recent_both)
...
jpm_check <- felm(
  business_days ~ 1 + 
    ...
    non_essential_closure_apr +
    non_essential_closure_current + 
       jpm + I(jpm * minority_percent)|0|0|zip, 
    data = most_recent_both)
...
national_check <- felm(
  business_days ~ 1 + 
    ...
    non_essential_closure_apr +
    non_essential_closure_current + 
       national + I(national * minority_percent)|0|0|zip, 
    data = most_recent_both)
...
cu_check <- felm(
  business_days ~ 1 + 
    ...
    non_essential_closure_apr +
    non_essential_closure_current + 
       cu + I(cu * minority_percent)|0|0|zip, 
    data = most_recent_both)
...
# Same robustness check just for the indicators for roundtable status -- this is
# not in Gov 50, please ignore TFs thx.
...
business_roundtable_check <- felm(
 ...
 cook_pvi + 
   ...
   I(national * minority_percent)|0|0|zip, 
    data = most_recent_both)
...
business_roundtable_check2 <- felm(
  business_days ~ 1 + 
    ...
    non_essential_closure_current + 
      cu + national +
       business_roundtable + 
      I(business_roundtable * minority_percent)|0|0|zip, 
    data = most_recent_both)
...
# part because I thought to do this a little bit later than the others. This
# adds in deposit data to see if bank-level differences are completely
# explained by differences in size.

# Read the file in if you have to

most_recent_deposits <- readRDS('most_recent_zip_banksmatchd_1130.rds') %>%
  mutate(national = 0) %>%
  mutate(national = ifelse(str_detect(lender, pattern = "national association"),
                           1, national),
         national = ifelse(str_detect(lender, pattern = "n.a."),
...
# Second regression just includes an extra interaction term to make sure
# we've controlled for everything.
...
# original 3.7 million loan dataset, but without the lender-specific stuff
# All model specifications here are the same.
...
# regressions with the national and cu stuff built in -- this is the 
# original most recent dataset
...
```{r}
# One final robustness check -- replacing minority_percent with ownership
# percentage!!!
...
```{r}
county_rt1_biz <- felm(business_days ~ 1 + 
    median_income + 
    ...
    national + cu + 
    business_roundtable + 
    I(business_roundtable * percent_minority_biz)|0|0|fips, 
    data = most_recent_ownership
)

county_rt2_biz <- felm(business_days ~ 1 + 
    median_income + 
    percent_minority_biz + 
    high_school_pct + 
    married_percent + male_percent + 
    ...
    national + cu + 
    business_roundtable + 
    I(business_roundtable * percent_minority_biz) + 
    I(national * percent_minority_biz)|0|0|fips, 
    data = most_recent_ownership
)
```

```{r}
# And let's run the bank robustness checks too

baml_biz <- felm(business_days ~ 1 + 
    median_income + 
    percent_minority_biz + 
    high_school_pct + 
    married_percent + male_percent + 
    ...
    non_essential_closure_current + 
    national + cu + 
    baml + I(baml * percent_minority_biz)|0|0|fips, 
    data = most_recent_ownership
)

citi_biz <- felm(business_days ~ 1 + 
    median_income + 
    percent_minority_biz + 
    high_school_pct + 
    married_percent + male_percent + 
    ...
    non_essential_closure_current + 
    national + cu + 
    citi + I(citi * percent_minority_biz)|0|0|fips, 
    data = most_recent_ownership
)
...
jpm_biz <- felm(business_days ~ 1 + 
    ...
    non_essential_closure_current + 
    national + cu + 
    jpm + I(jpm * percent_minority_biz)|0|0|fips, 
    data = most_recent_ownership
)
```

```{r}
wells_biz <- felm(business_days ~ 1 + 
    median_income + 
    percent_minority_biz + 
    high_school_pct + 
    married_percent + male_percent + 
    ...
    non_essential_closure_current + 
    national + cu + 
    wells + I(wells * percent_minority_biz)|0|0|fips, 
    data = most_recent_ownership
)
```
...
national_biz <- felm(business_days ~ 1 + 
    median_income + 
    percent_minority_biz + 
    high_school_pct + 
    married_percent + male_percent + 
    ...
    non_essential_closure_apr + 
    non_essential_closure_current + 
    national + I(national * percent_minority_biz)|0|0|fips, 
    data = most_recent_ownership
)

cu_biz <- felm(business_days ~ 1 + 
    median_income + 
    percent_minority_biz + 
    high_school_pct + 
    married_percent + male_percent + 
    ...
    non_essential_closure_apr + 
    non_essential_closure_current + 
    cu + I(cu * percent_minority_biz)|0|0|fips, 
    data = most_recent_ownership
)
```
```{r}
