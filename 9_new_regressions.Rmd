...
zip_county_crosswalk <- read_excel('raw_data/crosswalks/ZIP_COUNTY_092020.xlsx') %>%
  clean_names() %>%
...
```{r}
# adding in county level data to the ZIP for robustness check
...
demographics <- felm(business_days ~ 1 + 
    ...
    rural +
    estimate_gini_index +
    violent_crime_rate|fips + lender|0|zip, data = most_recent)
...
loan_spec <- felm(
  business_days ~ 1 + 
    ...
    loan_2mil_5mil + 
    loan_5mil_10mil + 
    jobs_reported|fips + lender|0|zip, data = most_recent)
...
econ <- felm(
  business_days ~ 1 + 
    ...
    unemployment_rate|fips + lender|0|zip, data = most_recent)
...
loan_spec_check <- felm(
  business_days ~ 1 + 
    ...
    cu +
    bank_ppp_issued + 
    jobs_reported|0|0|zip, data = most_recent_both)
...
econ_check <- felm(
  business_days ~ 1 + 
    ...
    county_unemployment_rate|0|0|zip, data = most_recent_both)
...
covid_check <- felm(
  business_days ~ 1 + 
    ...
    stay_at_home_current + 
    non_essential_closure_apr + 
    non_essential_closure_current|0|0|zip,
...
interactions_check <- felm(
  business_days ~ 1 + 
    ...
    I(minority_percent * rural) + 
    I(minority_percent * estimate_gini_index) +
    I(minority_percent * county_crime_rate)|0|0|zip,
  data = most_recent_both)
```
