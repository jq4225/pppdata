...
# These are the regressions that solely focus on aggregate disparities, rather
# than disaggregating by bank.
...
# Adding in FIPS to county, since it wasn't added in before.
...
crime <- read_excel('raw_data/county/county_health.xlsx', 
                    ...
                    skip = 1) %>%
  clean_names() %>%
  rename("county_crime_rate" = "violent_crime_rate + renter_percent") %>%
...
```{r}
# Zip with fixed effects -- regressions start here. FELM allows me to include
# fixed effects regressions, which I later implement (and you can see
# what the FEs are by what variables are after the first bar in 
# my formula.) I'm running a bunch of different model specifications 
# including and excluding certain categories of variables, as you can 
# see by the name. 
...
demographics <- felm(business_days ~ 1 + 
    ...
    rural +
    estimate_gini_index +
    violent_crime_rate + renter_percent|0|0|zip, data = most_recent)
...
```{r}
# Quick residuals plot for the Shiny App -- save this with ggsave

resid <- covid_fe$residuals

resid <- cbind(resid, covid_fe$response) %>%
  as_tibble() %>%
  rename("residuals" = "business_days",
         "business_days" = "V2") %>% 
  mutate(residuals = abs(residuals))

resid %>%
  ggplot(aes(x = business_days, y = residuals)) + 
    geom_jitter(alpha = 0.3, color = "darkblue") + 
    labs(x = "Waiting Time (business days)", y = "Residuals", 
         title = "Waiting Time vs. Residuals") +
    ylim(0, 70) + 
    theme_classic()
```
...
most_recent_both <- readRDS('most_recent_zip_withcounties_nobanks_1130.rds') %>%
  mutate(zip = as.factor(zip), fips = as.factor(fips))

# Robustness check -- rerunning regressions with the ZIP-level estimates 
# substituted for original county data. This means I take out county FEs
# but I do try out state FEs at some point, tho I don't keep them.
...
demographics_check <- felm(business_days ~ 1 + 
    ...
    rural +
    estimate_gini_index +
    county_crime_rate + renter_percent|0|0|zip, data = most_recent_both)
...
loan_spec_check <- felm(
  business_days ~ 1 + 
    ...
    rural +
    estimate_gini_index +
    county_crime_rate + renter_percent + 
    delay + 
    loan_150_350 +
    loan_350_1mil +
    loan_1mil_2mil +
    loan_2mil_5mil + 
    loan_5mil_10mil + 
    jobs_reported|lender|0|zip, data = most_recent_both)
...
econ_check <- felm(
  business_days ~ 1 + 
    ...
    county_unemployment_rate|lender|0|zip, data = most_recent_both)
...
sq_check <- felm(
  business_days ~ 1 + 
    ...
    non_essential_closure_apr + 
    non_essential_closure_current +
    I(minority_percent^2)|lender|0|zip,
  data = most_recent_both)
...
interactions_check <- felm(
  business_days ~ 1 + 
    ...
    I(minority_percent * rural) + 
    I(minority_percent * estimate_gini_index) +
    I(minority_percent * county_crime_rate)|lender|0|zip,
  data = most_recent_both)
```
...
# County fixed effects gone -- this is county regressions but without 
# state fixed effects -- I'll come back and rerun with state fixed effects
# at some point, I'm sure. Actually, state FEs work a little bit worse than
# the state dummies for COVID policies, so I end up including some regressions
# showing state FEs but not as my primary specification.
...
county_dems <- felm(business_days ~ 1 + 
    ...
    pop_den +
    estimate_gini_index +
    violent_crime_rate + renter_percent|0|0|fips, 
...
# Interactions differs b/c I include a bunch of interaction terms, not just
# one.
...
```
```{r}
# Trying another way of doing negative binomial w/ fixed effects -- this takes
# a while but ends up running properly! Negative binomial reg is for
# count data, which mine is given it's waiting times. It should help 
# correct for the skewness of my data (though the results don't really
# work that well).
...
dems_binomial <- fenegbin(business_days ~ 1 + 
    ...
    rural +
    estimate_gini_index +
    violent_crime_rate + renter_percent|fips, most_recent)
...
# You'll note that I don't really have interactions here, because including
# interactions makes my regressions take SO long that it's not worth it.
```

```{r}
# Trying log again just to see what will happen -- note R squared is better!
# The interpretation on this is probably a bit more reasonable than the straight
# linear regression, but this is still just a robustness check.
...
# Robustness check -- using state FEs instead of county FEs on the state level
# This actually ends up working fairly similarly, though obviously worse
# b/c there are just more county FEs.
...
covid_state <- felm(
  business_days ~ 1 + 
    ...
    stay_at_home_current + 
    non_essential_closure_apr + 
    non_essential_closure_current|lender + state|0|zip,
  data = most_recent)
...
sq_state <- felm(
  business_days ~ 1 + 
    ...
    non_essential_closure_apr + 
    non_essential_closure_current +
    I(minority_percent^2)|state + lender|0|zip,
  data = most_recent)

# Regression 6: interactions

interactions_state <- felm(
  business_days ~ 1 + 
    median_family_income + 
    minority_percent +
    high_school_pct + 
    ...
    I(minority_percent * rural) + 
    I(minority_percent * estimate_gini_index) +
    I(minority_percent * violent_crime_rate )|state + lender|0|zip,
  data = most_recent)
```

```{r}
# Let's also take a look at whether this relationship is significant on
# county level, using minority business ownership instead of minority
# percent -- this means we take out the state dummies

most_recent_ownership_county <- 
  readRDS('most_recent_county_nobanks_ownership.rds') %>%
  mutate(lender = as.factor(lender), state = as.factor(state),
         fips = as.factor(fips))

# For Justin -- add the other regressions here for a full table, then 
# rerun the business roundtable and bank in the next file too using this

county_dems_biz <- felm(business_days ~ 1 + 
    median_income + 
    ...
    estimate_gini_index +
    violent_crime_rate + renter_percent|0|0|fips, 
    data = most_recent_ownership_county
)

county_loan_biz <- felm(business_days ~ 1 + 
    median_income + 
    percent_minority_biz + 
    high_school_pct + 
    married_percent + male_percent + 
...
county_econ_biz <- felm(business_days ~ 1 + 
    median_income + 
    percent_minority_biz + 
    high_school_pct + 
    married_percent + male_percent + 
...
county_covid_biz <- felm(business_days ~ 1 + 
    median_income + 
    percent_minority_biz + 
    high_school_pct + 
    married_percent + male_percent + 
...
# Ran these already -- check Macbook GDrive
...
county_covid_biz <- felm(business_days ~ 1 + 
    ...
    mean_start_deaths + 
    mean_end_cases +
    mean_end_deaths|lender + state|0|fips, 
    data = most_recent_ownership_county
)
```
...
county_covid_biz_sq <- felm(business_days ~ 1 + 
    median_income + 
    percent_minority_biz + 
    high_school_pct + 
    married_percent + male_percent + 
    ...
    mean_end_cases +
    mean_end_deaths + 
    I(percent_minority_biz^2)|lender + state|0|fips, 
    data = most_recent_ownership_county
)

# No state FEs to check the fit -- this actually works better than the ones
# with state FEs.
...
county_covid_biz_sq <- felm(business_days ~ 1 + 
    ...
    non_essential_closure_apr + 
    non_essential_closure_current + 
    I(percent_minority_biz^2)|lender|0|fips, 
    data = most_recent_ownership_county
)
```
```{r}

# Testing out an IV because I can

# covid_fe <- felm(
#   business_days ~ 1 + 
#     median_family_income + 
#     high_school_pct +  male_percent + 
#     rural +
#     estimate_gini_index +
#     violent_crime_rate + renter_percent + 
#     delay +
...
#     (minority_percent ~ married_percent + cook_pvi)|zip,
#   data = most_recent)
```

```{r}

# Again trying something for a different graph later -- adding in state FEs
# and state * minority_percent interactions
...
covid_fe <- felm(
  log_days ~ 1 + 
    ...
    mean_end_cases_weighted +
    mean_end_deaths_weighted + 
    state:minority_percent|lender + state|0|zip,
  data = most_recent)

```
```{r}
# Re-running these regressions, scaling to standard deviations -- we first scale
# everything by its standard deviation

most_recent_sd <- most_recent %>%
  mutate_at(c("business_days",
              "median_family_income",
              "minority_percent",
              "high_school_pct",
              "married_percent",
...
# And now we run the regressions for ZIP
...
```{r}
# Do the same thing on the county level

most_recent_county <- readRDS('most_recent_county_nobanks_ownership.rds') %>%
  mutate(lender = as.factor(lender), fips = as.factor(fips)) %>%
  mutate_at(c("business_days",
              "median_income",
              "percent_minority_biz",
              "high_school_pct",
...
```{r}
# One more check -- I want to see if minority business ownership is significnant
# controlling for population

county_covid_biz <- felm(business_days ~ 1 + 
    median_income + 
    percent_minority_biz + minority_percent + 
    high_school_pct + 
    married_percent + male_percent + 
    republican_percent + 
    pop_den +
